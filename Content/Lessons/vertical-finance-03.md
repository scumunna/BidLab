...
Google runs a parallel rail through G2 (G2 Risk Solutions), but scope it carefully. The G2 program covers in-scope financial-services advertisers in certain non-US markets (for example India, Singapore, Indonesia, and Malaysia, with Malaysia effective April 14, 2026), not US finance inventory. In a covered market, advertisers must verify through G2, proving that a regulator directly authorizes them (or that they are exempt) plus a registration number, then submit the unique G2 code to Google. Until that code is on file, unverified advertisers are blocked from in-scope finance inventory. US finance advertisers instead face Google's separate, older "financial products and services" advertiser-verification process, not the G2RS program. For the covered Malaysia rollout, G2 began processing financial-services verification applications on March 10, 2026, with enforcement starting April 14, 2026, so onboarding lead time matters for advertisers targeting G2-covered countries, not as an afterthought.
...
The third rail is federal law, not platform policy. Any audience built from a consumer report or a prescreen is governed by the Fair Credit Reporting Act (FCRA). Such an audience is permissible only if it results in a firm offer of credit (or insurance). Each name on a prescreen list counts as a consumer report that requires a permissible purpose, and a Prescreened Opt-Out Notice must accompany the offer. "Trigger leads," signals that someone is actively shopping for credit, are now largely prohibited under the FCRA as amended by the Homebuyers Privacy Protection Act (signed September 5, 2025, effective March 4, 2026). A consumer reporting agency may furnish a mortgage trigger lead only if it is a firm offer of credit or insurance and the requestor either has the consumer's documented opt-in consent or a qualifying existing relationship (the consumer's current originator or servicer, or the consumer's depository institution or credit union). Treat trigger-lead audiences as statutorily restricted, not just high-scrutiny.
